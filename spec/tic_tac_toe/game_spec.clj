...
(describe "Game"
  (context "#switch-player"
    (it "returns the opposite marker"
      (let [[player1 player2] (:players (game/initial-state 3))]
        (should= player2 (game/switch-player player1 (game/initial-state 3)))
        (should= player1 (game/switch-player player2 (game/initial-state 3))))))

  (context "game-state"
    (it "has a board"
      (should= (board/initial-board 3) (:board (game/initial-state 3))))

    (it "has a turn counter"
      (should= 0 (:turn-counter (game/initial-state 3))))

   (context "#set-turn-counter"
     (it "returns 0 given an empty board"
       (should= 0 (game/set-turn-counter (board/initial-board 3)))))

   (context "#update-board"
     (it "creates a copy of the current game state with updated board and turn counter"
       (let [updated-state (game/update-board (game/initial-state 3) first-move-x)]
         (should= first-move-x (:board updated-state))
         (should=
          (inc (:turn-counter (game/initial-state 3))) (:turn-counter updated-state)))))
   ...
   (context "#progress-game-state"
     (it "returns a state with an increased turn counter"
       (should= (inc (:turn-counter (game/initial-state 3)))
                (:turn-counter (game/progress-game-state 0 (game/initial-state 3)))))

     (it "returns a state with an updated board"
       (should= (board/place-marker (:board (game/initial-state 3)) 0 "X")
                (:board (game/progress-game-state 0 (game/initial-state 3))))))

   (context "#current-player"
     (it "returns the current player based on the first player and the game state"
       (should= (player/human "X") (game/current-player (game/initial-state 3)))))
